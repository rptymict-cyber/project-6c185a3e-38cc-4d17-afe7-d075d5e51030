        ...
        <div className="overflow-hidden rounded-2xl border border-[#E9ECEF] bg-white">
          ...
          <div className="flex items-center gap-3 bg-gradient-to-br from-[#EAF3FB] to-[#F3F8FC] px-3 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 flex items-baseline gap-1.5">
                ...
                <span className="text-meta text-[#495057]">
            ...
            <div className="flex items-center gap-2">
              {MOCK_WEATHER.forecast.map((f) => (
                ...
                >
                  <span className="text-meta font-semibold text-[#6C757D]">
                    {f.label}
                  </span>
                  <span className="text-body leading-none">{f.icon}</span>
                  <span className="mt-0.5 text-meta font-bold tabular-nums text-[#111827]">
                    {f.temp}°
                  </span>
                </div>
              ))}
            ...
            {MOCK_WEATHER.cause.steps.map((s, i) => (
              <div key={i} className="flex flex-1 items-center gap-1">
                ...
                >
                  ...
                  <span className="mt-0.5 text-meta text-[#6C757D]">
                    {s.sm}
                  </span>
                </div>
                {i < MOCK_WEATHER.cause.steps.length - 1 && (
                  <span className="text-body font-bold text-[#ADB5BD]">
                    →
                  </span>
                )}
              </div>
          ...
          <div className="bg-gradient-to-r from-[#2E9E6B] to-[#1F7A50] px-3 py-2.5 text-white">
            <div className="flex items-center gap-2">
              ...
              <span className="text-caption font-semibold opacity-90">
                예상 결과
              </span>
              <span className="rounded-md bg-white/20 px-2 py-0.5 text-caption font-extrabold">
                {MOCK_WEATHER.cause.result}
              </span>
            </div>
            <div className="mt-1 text-meta text-white/80">
              ※ 틸다 예측 모델 반영 요인
            </div>
          </div>
        </div>
