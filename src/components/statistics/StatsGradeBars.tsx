...
export function StatsGradeBars({ rows }: { rows: GradeRow[] }) {
  ...
  return (
    <div className="rounded-[12px] border border-[#E9ECEF] bg-white p-3">
      <h4 className="text-body font-bold text-foreground">등급별 평균가</h4>
      <ul className="mt-3 space-y-2.5">
        {rows.map((r) => {
          const w = Math.max(6, Math.round((r.price / max) * 100));
          return (
            <li key={r.grade} className="flex items-center gap-2">
              <span className="w-8 shrink-0 text-caption font-bold text-[#495057]">{r.grade}</span>
              <div className="relative flex-1">
                ...
                <span className="absolute left-2 top-1/2 -translate-y-1/2 text-meta font-bold text-white mix-blend-plus-lighter">
                  {r.price.toLocaleString()}원/kg
                </span>
              </div>
              <span className="w-12 shrink-0 text-right text-meta font-semibold tabular-nums text-[#6C757D]">
                {r.share}%
              </span>
            </li>
          );
      ...
      <p className="mt-3 text-meta leading-snug text-[#868E96]">
