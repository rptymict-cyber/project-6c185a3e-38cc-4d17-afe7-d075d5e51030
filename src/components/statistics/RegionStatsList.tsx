  ...
  return (
    <div className="px-4">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-body-lg font-bold">지역별 평균가 목록</div>
          <div className="mt-0.5 text-meta text-[#868E96]">시·도 {regions.length}곳</div>
        </div>
        <select
          value={sort}
          onChange={(e) => setSort(e.target.value as Sort)}
          className="rounded-[8px] border border-[#E9ECEF] bg-white px-2 py-1.5 text-caption font-semibold text-[#495057]"
        >
          {SORT_OPTIONS.map((o) => (
      ...
      <div className="mt-3 flex items-center gap-2 rounded-t-[12px] border border-b-0 border-[#E9ECEF] bg-[#F8F9FA] px-3 py-2 text-meta font-bold text-[#6C757D]">
          ...
          return (
            <li key={r.region} className={cn(active && "bg-[#F5FBF5]")}>
              ...
              >
                <span
                  className={cn(
                    "w-5 text-caption font-bold tabular-nums",
                ...
                <span className="text-body font-bold tabular-nums">
                  {r.avgKg.toLocaleString()}<span className="text-meta font-semibold text-[#868E96]">원</span>
                </span>
                <span
                  className={cn(
                ...
                <span className="w-14 text-right text-meta text-[#868E96] tabular-nums">
                  {r.volumeTon.toFixed(1)}t
                </span>
                <span className="hidden xs:block text-meta text-[#ADB5BD]">
              ...
              {open && (
                <ul className="border-t border-[#F1F3F5] bg-[#FBFDFB] px-3 pb-2">
                  <li className="flex items-center gap-2 pt-2 pb-1 text-meta font-bold text-[#868E96]">
                    ...
                    return (
                      <li key={m.id}>
                        ...
                        >
                          ...
                          <span className="text-body font-bold tabular-nums">
                            {m.avgKg.toLocaleString()}<span className="text-meta text-[#868E96]">원</span>
                          </span>
                          <span
                            className={cn(
                        ...
                        {mOpen && m.companies.length > 0 && (
                          <div className="mb-1 rounded-[8px] bg-white px-2 py-2">
                            <div className="flex items-center gap-2 pb-1 text-meta font-bold text-[#868E96]">
