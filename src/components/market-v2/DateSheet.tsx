...
type TradingDay = {
  ...
  weekday: string; // "토"
  hasData: boolean;
  disabled?: boolean; // 휴장·미래
};

// Deterministic mock week for scaffolding (matches prompt: 7/5(토) recent).
...
const QUICKS: { key: string; iso: string; title: string; sub: string }[] = [
...
export function DateSheet({
  ...
  const { date, setDate } = useMarketFilter();

  const selected = date;
  const quicks = useMemo(() => QUICKS, []);

  const commit = (iso: string, humanLabel: string) => {
    setDate(iso, humanLabel);
    onOpenChange(false);
  };
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="rounded-t-2xl p-0">
        ...
        <div className="grid grid-cols-3 gap-2 px-5 pt-4">
          {quicks.map((q) => {
            const active = selected === q.iso && q.key === "recent"; // recent default active
            return (
              <button
                key={q.key}
                onClick={() =>
                  commit(
                    q.iso,
                    `${q.sub.replace(" ", " ")} · ${q.title}`,
                  )
                }
                className={cn(
                  "rounded-[10px] border px-2 py-2.5 text-center",
                  active
                    ? "border-[#3A8A3A] bg-[#F0F9F0]"
