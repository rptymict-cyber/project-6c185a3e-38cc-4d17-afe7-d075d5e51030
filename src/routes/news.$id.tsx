  ...
  return (
    <AppShell screenId="NEWS-002_뉴스상세"
      header={
        <div className="flex h-12 items-center justify-between border-b border-[#F1F3F5] bg-white px-2">
          ...
          <div className="text-body font-bold text-foreground">농업 뉴스</div>
    ...
    >
      <article className="px-5 pb-24 pt-4">
        <span className="inline-flex items-center gap-0.5 rounded-md bg-[#F0EBFF] px-2 py-0.5 text-meta font-bold text-[#6741D9]">
          ✨ AI를 통해 작성된 기사
        </span>
        ...
        <div className="mt-2 text-caption text-[#6C757D]">
        ...
        <div className="mt-5 space-y-[13px]">
          {item.body?.map((p, i) => (
            <p
              ...
              className={
                i === 0
                  ? "text-body font-bold leading-[1.85] text-foreground"
                  : "text-body leading-[1.85] text-[#343A40]"
              }
            >
              {p}
            </p>
        ...
        {basis && (
          <section className="mt-6 rounded-2xl border border-[#D8E9E0] bg-[#F3FAF6] p-3.5">
            <div className="text-body font-bold text-[#1F7A50]">
              📊 이 기사의 근거 데이터
            </div>
            <p className="mt-1 text-caption leading-snug text-[#495057]">
            ...
            <div className="mt-2.5 flex flex-wrap gap-1.5">
              {basis.crops.map((c) => (
                <span
                  key={c}
                  className="rounded-full border border-[#2E9E6B] bg-white px-2.5 py-1 text-meta font-semibold text-[#1F7A50]"
                >
                  🧅 {c}
                </span>
              ))}
              {basis.market && (
                <span className="rounded-full border border-[#2E9E6B] bg-white px-2.5 py-1 text-meta font-semibold text-[#1F7A50]">
                  {basis.market}
                </span>
              )}
              {basis.period && (
        ...
        <div className="mt-4 rounded-xl bg-[#F8F9FA] px-3 py-2 text-meta leading-snug text-[#6C757D]">
        ...
        {basis?.crops && basis.crops.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {basis.crops.map((c) => (
              <span key={c} className="text-caption text-[#6C757D]">
                #{c}
              </span>
            ))}
          </div>
